...
def test_scaffold_validates_unchanged(tmp_path, offline):
    path = write_scaffold("acme-portal", offline=offline, directory=tmp_path)
    spec = load_spec(path)
...
def test_offline_scaffold_lists_images(tmp_path):
    spec = load_spec(write_scaffold("acme-portal", offline=True, directory=tmp_path))
...
def test_online_scaffold_names_a_registry(tmp_path):
    spec = load_spec(write_scaffold("acme-portal", offline=False, directory=tmp_path))
...
def test_scaffold_writes_readable_json(tmp_path):
    path = write_scaffold("acme-portal", offline=False, directory=tmp_path)
    text = path.read_text()
...
def test_init_refuses_to_clobber_an_edited_spec(tmp_path):
    write_scaffold("acme-portal", offline=False, directory=tmp_path)
...
def test_init_rejects_names_that_cannot_be_a_package(tmp_path, name):
    with pytest.raises(SpecError, match="Debian package name"):
        write_scaffold(name, offline=False, directory=tmp_path)


def test_init_rejects_a_bad_name_before_creating_anything(tmp_path):
    with pytest.raises(SpecError):
        write_scaffold("Acme", offline=False, directory=tmp_path)
